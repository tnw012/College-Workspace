def test_binpacking(num_list, capacity, test_number=100):
    ...
    for index in range(3):
        ...
        if index == 0:
            ...
            print("next-fit number of bins required: ", next_bin_total, "\n")
            
        elif (index == 1):
            print("Testing NP-Completeness first-fit:")

            # loop from 0 to test_number(number of tests)
            for i in range(test_number):
                start_time = time.perf_counter()
                
                next_bin_total = bp.first_fit(num_list, capacity, len(num_list))
            ...
            print("The average time out of %i tests = %.4f ms" % (test_number, average_time))
            print("first-fit number of bins required: ", next_bin_total, "\n")
            
        else:
            print("Testing NP-Completeness best-fit:")
